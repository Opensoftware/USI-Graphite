class Graphite::Xlsx::AcceptedStudentList < Graphite::Xlsx::StudentList
  ...
  def initialize(students)
    ...
    .includes(:user,
              :student_studies => [:studies => [:course => :translations,
              ...
              :accepted_elective_enrollments => [:block => :translations,
                                                 :elective_module => :translations,
                                                 :elective_block => [:elective_block_studies,
                                                                     :translations, :semester]]).load
  end
      ...
      student.accepted_elective_enrollments.each do |enrollment|
        ...
        if student_studies.present?
          row = [
            ...
            enrollment.elective_block.semester.try(:name),
